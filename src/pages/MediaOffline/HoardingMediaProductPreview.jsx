...
function HoardingImageCarousel({ ImageDataArray }) {
...
export default function HoardingMediaProductPreview() {
  const { id } = useParams();
  ...
  const hoardingColumns = [
    ...
    { field: 'landmark', headerName: 'Traffic', width: 130,
      valueGetter: (value, row) => row?.landmark ?? row?.Landmark ?? row?.traffic ?? row?.Traffic ?? value ?? '',
    },
    { field: 'mediaType', headerName: 'Type', width: 120 },
    { field: 'latitude', headerName: 'Lat', width: 90 },
    { field: 'longitude', headerName: 'Long', width: 90,
      valueGetter: (value, row) => row?.longitude ?? row?.Longitude ?? row?.long ?? row?.Long ?? value ?? '',
    },
          ...
          <Grid container>
            ...
            >
              {!ImageDataArray?.length ? (
                ...
                <HoardingImageCarousel ImageDataArray={ImageDataArray} />
              )}
            </Grid>
                    ...
                    {GetProductByIdData?.OtherCost &&
                      GetProductByIdData?.OtherCost?.length !== 0 ? (
                        <Box mt={2}>
                          ...
                          {GetProductByIdData?.OtherCost?.length === 0
                            ? ''
                            : GetProductByIdData?.OtherCost?.map((cost, costIdx) => {
                              const newValue = cost?.CostPrice.toFixed(2);
                              return (
                                <Box
                                  key={cost?._id != null ? String(cost._id) : `other-cost-${costIdx}`}
                                  sx={{
                                    display: 'flex',
                                    gap: '60px',
                                    mt: 1,
                            ...
                            >
                              {GetProductByIdData?.OtherInformationBuyerMustKnowOrRemarks.map(
                                (item, id) => (
                                  <Typography
                                    key={`remark-${id}`}
                                    sx={{ ...otherCostText, width:'80vw', wordWrap:'break-word' }}
                                  >
                                    {id + 1}) {item},{' '}
                                  </Typography>
                                ),
                      ...
                      <Box mt={2}>
                        ...
                        {GetProductByIdData?.OtherCost?.length === 0
                          ...
                          : GetProductByIdData?.OtherCost?.map((cost, costIdx) => {
                            ...
                            return (
                              <Box
                                key={cost?._id != null ? String(cost._id) : `other-cost-tab-${costIdx}`}
                                sx={{
                                  display: 'flex',
                                  gap: '60px',
                                  mt: 1,
                      ...
                      <>
                        <Box sx={{ mt: 3 }}>
                          ...
                          >
                            {GetProductByIdData?.OtherInformationBuyerMustKnowOrRemarks.map(
                              (item, remarkIdx) => (
                                <Typography key={`remark-tab-${remarkIdx}`} sx={otherCostText}>
                                  {item}
                                </Typography>
                              ),
                            )}
